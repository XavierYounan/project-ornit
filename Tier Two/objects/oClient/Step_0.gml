...
switch state
{
	...
	case state.readyUp:
	{
		...
		if (m_framesTillUpdate = 0)
		{
			...
			var packet = gnet_packet_build(PACKET_IDENTIFIER.T2_SELF_POSITION,x,y,m_ClientId,hero.m_lastProcessedImput[2])
	...
	{		
		if (m_framesTillUpdate <= 0)
		{
			...
			var packet = gnet_packet_build(PACKET_IDENTIFIER.T2_SELF_POSITION,x,y,m_ClientId,hero.m_lastProcessedImput[2],hero.hsp,hero.vsp)
			gnet_packet_send_to_id(packet,m_ClientId)
